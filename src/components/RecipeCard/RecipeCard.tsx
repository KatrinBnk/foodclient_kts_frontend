import { ShortRecipe } from '@/types';
import styles from './RecipeCard.module.scss';
import React from 'react';

interface RecipeCardProps {
  recipe: ShortRecipe;
  onSave: (documentId: string) => void;
  onCardClick: (documentId: string) => void;
}

const RecipeCard: React.FC<RecipeCardProps> = ({ recipe, onSave, onCardClick }) => {
  const ingredients = recipe.ingradients
    ? recipe.ingradients.map((item) => item.name).join(' + ')
    : 'Ingredients not specified';

  return (
    <div className={styles.card} onClick={() => onCardClick(recipe.documentId)}>
      <div className={styles.imageWrapper}>
        {recipe.images && recipe.images.length > 0 && (
          <img src={recipe.images[0].url} alt={recipe.name} className={styles.image} />
        )}
      </div>

      <div className={styles.content}>
        <div className={styles.textBlock}>
          <div className={styles.time}>
            <span className={styles.timeIcon}>
              {/* TODO: Вынести иконку в отдельный компонент */}
              <svg
                width="14"
                height="14"
                viewBox="0 0 14 14"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M11.1818 1L13 2.81818M10.8182 10.8182L12.2727 13M2.81818 1L1 2.81818M3.18182 10.8182L1.72727 13M6.81818 3.90909V7.18182H8.63636M12.2727 7C12.2727 9.91207 9.91207 12.2727 7 12.2727C4.08795 12.2727 1.72727 9.91207 1.72727 7C1.72727 4.08796 4.08795 1.72727 7 1.72727C9.91207 1.72727 12.2727 4.08796 12.2727 7Z"
                  stroke="#B5460F"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </span>
            <span className={styles.timeText}>{recipe.totalTime} minutes</span>
          </div>
          <h3 className={styles.title}>{recipe.name}</h3>
          <p className={styles.ingredients}>{ingredients}</p>
        </div>

        <div className={styles.footer}>
          <span className={styles.calories}>{Math.round(recipe.calories)} kcal</span>
          <button className={styles.saveButton} onClick={() => onSave(recipe.documentId)}>
            Save
          </button>
        </div>
      </div>
    </div>
  );
};

export default RecipeCard;
